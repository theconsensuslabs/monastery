...
t2: select * from test; -- T2
t2: update test set value = 12 where id = 1; -- T2
t2: update test set value = 18 where id = 2; -- T2
t2: commit; -- T2

t1: delete from test where value = 20; -- T1. Prints "ERROR: could not serialize access due to concurrent update"
t1: abort; -- T1. There's nothing else we can do, this transaction has failed
