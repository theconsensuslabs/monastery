...
t2: update test set value = 12 where id = 1;       -- group final; assert ok or t2aborted => error
t1: commit;
t3: select * from test where id = 1; -- assert ({1, 11}) or ({1, 12})
t2: update test set value = 18 where id = 2;       -- group final; assert ok or t2aborted => error
